...
class HashToServoLookup:
    ...
    def load_lookup_tables(self):
        ...
        try:
            ...
            if not lookup_path.exists():
                logging.warning(
                    f"Hash lookup file {self.lookup_file} not found!")
            ...
            total_mappings = metadata.get(
                'total_mappings', len(self.hash_to_servo))
            step_size = metadata.get(
                'servo_step_size', metadata.get('step_size', 'unknown'))

            logging.info(
                f"Loaded {len(self.hash_to_servo)} hash-to-servo mappings from {self.lookup_file}")
            ...
            logging.error(
                f"Error loading hash lookup file {self.lookup_file}: {e}")
    ...
    def _fallback_to_inference_data(self):
        ...
        try:
            logging.info(
                "Attempting to load hash data from inference_data.json as fallback...")
            ...
            if not self.hash_to_servo:
                logging.info(
                    "No hash lookup in metadata, creating from samples...")
                self._create_lookup_from_samples(data)

            logging.info(
                f"Fallback: Loaded {len(self.hash_to_servo)} hash mappings from inference data")
    ...
    def get_servo_angles_from_hash(self, position_hash):
        ...
        if servo_angles is None:
            ...
            if closest_hash:
                ...
                logging.info(
                    f"Generated fallback servo angles: {servo_angles}")
    ...
    def _find_closest_hash(self, target_hash):
        ...
        for hash_candidate in available_hashes:
            score = sum(1 for a, b in zip(
                target_hash, hash_candidate) if a == b)
            if score > best_score:
                best_score = score
    ...
    def list_available_hashes(self):
        ...
        available_hashes = list(self.hash_to_servo.keys())
        logging.info(
            f"Available hashes ({len(available_hashes)} total): {available_hashes[:5]}...")
...
class Action:
    ...
    def _ensure_inference_data_exists():
        ...
        if not inference_file.exists() or not inference_file.is_file():
            logging.info(
                "Inference data file not found. Generating inference dataset...")
            try:
                ...
                generator.save_dataset_to_json(
                    inference_dataset, 'inference_data.json')
    ...
    def perform_action(servo_controller: ServoController) -> None:
        ...
        try:
            ...
            logging.debug(
                f"Current Servo Positions: {servo_controller.get_current_positions()}")
            ...
            target_servo_angles = lookup.get_servo_angles_from_hash(
                predicted_hash)
            ...
            logging.info(
                f"Final Servo Positions: {servo_controller.get_current_positions()}")
...
if __name__ == "__main__":
    ...
    logging.info("-" * 50)
    logging.debug(
        "This is the Action Sequence. This is where the machine learning model interacts with the hardware.")
    logging.info("-" * 50)
    ...
    try:
        ...
        while running:
            if test_mode:
                user_input = input(
                    "Press Enter to test inference, 'l' to test lookup, 'q' to quit: ")
            else:
                user_input = input(
                    "Press Enter to Inference Another Action, 'q' to quit: ")
